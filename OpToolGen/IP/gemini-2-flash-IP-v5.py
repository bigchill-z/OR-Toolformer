...
def get_q(data):
  ...
  # Your audience is students who are learning operations research knowledge. Adjust your question so that they can be accurately understood and answered.
    ...
    prompt = f"""# CONTEXT #
You are a professional operations research teacher, skilled in designing problems related to operations research. To help students further master Integer Programming, we now need you to set a reasonable Integer Programming problem.
# OBJECTIVE #
<DATA INFORMATION>
...
Please flexibly develop a reasonable Integer Programming problem based on the above DATA INFORMATION. Prohibit the use of data other than the above-mentioned information! 
...
4. The question should include a real background, a detailed description of the data involved, specific questions. Do not include any variables! Prohibit explicitly mentioning that this is an Integer Programming problem! Prohibit explicitly mentioning the use of Integer Programming problem related methods for solving! Prohibit adding new constraints! Please note that BINARIES means that variables can only be equal to 0 or 1! Please strictly follow the information provided by CONSTRAINTS, especially for constraints on individual variables!
...
>>>
# RESPONSE #
A brief Integer Programming problem that is easy to understand. Use <question> and </question> to package the problem. And there is no other unrelated content.
"""
...
def gen_ip_problem(num_variables=3, num_constraints=2, int_coeff=None, coeff_range=(0, 10)):
    # 生成随机系数
    def generate_coeff():
        if int_coeff:
    ...
    return {
        ...
        'forTool':{
            ...
            'input':{
                ...
                'temp':[goal,'obj: ' + objective.replace('*', ' '),[re.sub(r"\(.*?\)", "", f'c{c + 1}: {constraints[c].replace("*", " ")}').strip(' ') for c in range(len(constraints))],bounds,binary_variables]
            }
        }
    }
...
def judge(data,answer,label_output):
    ...
    if label_tool == pred_tool:
        ...
        pred_input = eval(f'extract_tool_ip_input({pred_input})')
...
if __name__ == '__main__':
    ...
    def worker():
        while not work.empty():
            current = work.get()
            if len(result['system'])>=538:###############2130
                # 清空任务队列
                while not work.empty():
                    work.get()
                break
            try:
                data = gen_ip_problem(num_variables=random.randint(2,5),num_constraints=random.randint(2,5),coeff_range=(10,1000))
                ...
                if judge_result:
                    ...
                    if random.random() < 0.5:
                        ...
                        ques =  q.group(1).strip("\n").strip().replace("\n\n", "")
                    ans = answer.replace('```tool_code','').replace('json\n','').replace('```','').replace("\n\n\n","\n\n").strip('\n')
                    necessary_tools = []
...
Always use the data provided in the problem description to avoid fabricating any incorrect data. Remember to pass in accurate and complete parameters to the tool without omission!"""
                    ...
                    result_df.to_excel('/home/bigchill/mypipe/genChat5/IP/IP_Chat5.xlsx', index=False)
                else:
                    continue
