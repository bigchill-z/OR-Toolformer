...
def get_q(data):
  ...
  # Your audience is students who are learning operations research knowledge. Adjust your question so that they can be accurately understood and answered.
    ...
    prompt = f"""# CONTEXT #
You are a professional operations research teacher, skilled in designing problems related to operations research. To help students further master Linear Programming, we now need you to set a reasonable Linear Programming problem.
# OBJECTIVE #
<DATA INFORMATION>
...
Please flexibly develop a reasonable Linear Programming problem based on the above DATA INFORMATION. Prohibit the use of data other than the above-mentioned information! 
...
4. The question should include a real background, a detailed description of the data involved, specific questions. Do not include any variables! Prohibit explicitly mentioning that this is a Linear Programming problem! Prohibit explicitly mentioning the use of Linear Programming problem related methods for solving! Prohibit adding new constraints!
...
>>>
# RESPONSE #
A brief Linear Programming problem that is easy to understand. Use <question> and </question> to package the problem. And there is no other unrelated content.
"""
...
def gen_lp_problem(num_variables=3, num_constraints=2, int_coeff=None, coeff_range=(0, 10)):
    # 生成随机系数
    def generate_coeff():
        if int_coeff:
    ...
    candi_cons = [gen_demand_cons, gen_policy_regulatory_cons, gen_proportionality_cons,gen_demand_cons, gen_policy_regulatory_cons, gen_proportionality_cons,gen_demand_cons, gen_policy_regulatory_cons, gen_proportionality_cons,gen_balance_cons,]# gen_logical_cons适合IP MILP
    # 打乱顺序
    ...
    return {
        ...
        'forTool':{
            ...
            'input':{
                ...
                'temp':[goal,'obj: ' + objective.replace('*', ' '),[re.sub(r"\(.*?\)", "", f'c{c + 1}: {constraints[c].replace("*", " ")}').strip(' ') for c in range(len(constraints))],bounds]
            }
        }
    }
...
def judge(data,answer,label_output):
    ...
    if label_tool == pred_tool:
        ...
        pred_input = eval(f'extract_tool_lp_input({pred_input})')
...
if __name__ == '__main__':
    ...
    def worker():
        while not work.empty():
            current = work.get()
            if len(result['system'])>=5:####################2032
                # 清空任务队列
                while not work.empty():
                    work.get()
                break
            try:
                data = gen_lp_problem(num_variables=random.randint(2,5),num_constraints=random.randint(2,5),coeff_range=(10,1000))
...
Always use the data provided in the problem description to avoid fabricating any incorrect data. Remember to pass in accurate and complete parameters to the tool without omission!"""
                    ...
                    result_df.to_excel('/home/bigchill/mypipe/genChat5/LP/LP_Chat5.xlsx', index=False)
                else:
                    continue
